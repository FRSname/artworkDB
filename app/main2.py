...
async def api_create_artwork(
    ...
    """Create a new artwork via API with optional image uploads"""
    # Check if artwork_id already exists
    existing = session.exec(select(Artwork).where(Artwork.artwork_id == artwork_id)).first()
    if existing:
        raise HTTPException(400, f"Artwork ID {artwork_id} already exists")
    ...
    if images and images[0].filename:
        ...
        for idx, img in enumerate(images):
            if img.filename:
                ...
                session.add(image_record)
                uploaded_images.append(image_record.dict())
...
@app.post("/api/artworks/json")
async def api_create_artwork_json(
    artwork_data: dict,
    session: Session = Depends(get_session),
    _auth=Depends(check_api_key)
):
    """Create artwork via JSON payload (without image uploads)"""
    # Check if artwork_id already exists
    artwork_id = artwork_data.get("artwork_id")
    if not artwork_id:
        raise HTTPException(400, "artwork_id is required")
        
    existing = session.exec(select(Artwork).where(Artwork.artwork_id == artwork_id)).first()
    if existing:
        raise HTTPException(400, f"Artwork ID {artwork_id} already exists")
    
    # Create artwork from JSON data
    ...
    artwork = Artwork(
        artwork_id=artwork_id,
        title=title,
        artist_name=artwork_data.get("artist_name", "Vladislav Raszyk"),
        year=artwork_data.get("year", ""),
        medium=artwork_data.get("medium", ""),
        surface=artwork_data.get("surface", ""),
        width_cm=artwork_data.get("width_cm", 0.0),
        height_cm=artwork_data.get("height_cm", 0.0),
        depth_cm=artwork_data.get("depth_cm", 0.0),
        description=artwork_data.get("description", ""),
        keywords=artwork_data.get("keywords", ""),
        web_slug=mk_slug(title, artwork_data.get("artist_name", "Vladislav Raszyk"))
    )
...
async def api_update_artwork(
    ...
    if "artist_name" in artwork_data:
        artwork.artist_name = artwork_data["artist_name"]
    ...
    # Update slug if title or artist changed
    if "title" in artwork_data or "artist_name" in artwork_data:
        artwork.web_slug = mk_slug(artwork.title, artwork.artist_name)
